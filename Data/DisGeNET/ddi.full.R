sel <- read.csv("Discurate.full.csv", header=T, stringsAsFactors = F)
list <- unique(sel$ID)
...
write.table(disorder.matrix, file="ddi.full.csv", sep=",", col.names=F,
            row.names=F, quote=F)
...
png("ddi.full.log10.heatmap.png", width=10, height=9, res=600, units="in")

...
selected <- read.csv("selected.csv", header=T, stringsAsFactors = F)

id.list <- selected$DisGeNET
name.list <- selected$DisGeNET.name
...
write.table(out.data, file="DisGeNET.selected.csv", sep=",", col.names=F, row.names = F, quote=T)
...
#the Gene list
geneset <- read.csv("HALLMARK_E2F_TARGETS.csv", header=T, stringsAsFactors = F)
...
pdf("HALLMARK_E2F_TARGETS.dendro.pdf", width=10, height=4, paper='special')
plot_dendrogram(dendro, rect=0, ann=F)
dev.off()

cluster <- cluster_walktrap(sub.graph)
pdf("HALLMARK_E2F_TARGETS.custer.pdf", width=5, height=5, paper='special')

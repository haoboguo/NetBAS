...
#the Gene list
geneset <- read.csv("HALLMARK_MTORC1_SIGNALING.csv", header=T, stringsAsFactors = F)
...
pdf("HALLMARK_MTORC1_SIGNALING.dendro.pdf", width=10, height=4, paper='special')
plot_dendrogram(dendro, rect=0, ann=F)
dev.off()

cluster <- cluster_walktrap(sub.graph)
pdf("HALLMARK_MTORC1_SIGNALING.custer.pdf", width=5, height=5, paper='special')

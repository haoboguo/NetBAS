...
#the Gene list
geneset <- read.csv("HALLMARK_MYC_TARGETS_V1.csv", header=T, stringsAsFactors = F)
...
pdf("HALLMARK_MYC_TARGETS_V1.dendro.pdf", width=10, height=4, paper='special')
plot_dendrogram(dendro, rect=0, ann=F)
dev.off()

cluster <- cluster_walktrap(sub.graph)
pdf("HALLMARK_MYC_TARGETS_V1.custer.pdf", width=5, height=5, paper='special')

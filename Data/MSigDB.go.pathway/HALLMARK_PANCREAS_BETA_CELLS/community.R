...
#the Gene list
geneset <- read.csv("HALLMARK_PANCREAS_BETA_CELLS.csv", header=T, stringsAsFactors = F)
...
pdf("HALLMARK_PANCREAS_BETA_CELLS.dendro.pdf", width=10, height=4, paper='special')
plot_dendrogram(dendro, rect=0, ann=F)
dev.off()

cluster <- cluster_walktrap(sub.graph)
pdf("HALLMARK_PANCREAS_BETA_CELLS.custer.pdf", width=5, height=5, paper='special')

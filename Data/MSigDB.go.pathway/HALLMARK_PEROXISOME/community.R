...
#the Gene list
geneset <- read.csv("HALLMARK_PEROXISOME.csv", header=T, stringsAsFactors = F)
...
pdf("HALLMARK_PEROXISOME.dendro.pdf", width=10, height=4, paper='special')
plot_dendrogram(dendro, rect=0, ann=F)
dev.off()

cluster <- cluster_walktrap(sub.graph)
pdf("HALLMARK_PEROXISOME.custer.pdf", width=5, height=5, paper='special')

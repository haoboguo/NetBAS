...
hmk.summary <- c("HMK.set.name", "List.num", "Gene.num", "Vertice.num", "Edge.num",
                 ...
                 "PCC.DB", "PCC.DC", "PCC.BC", "Clust.Coeff.Global", "Clust.Coeff.Average",
...
for (i in 1:length(hmk.set.list)) {
  ...
  kegg.file <- paste(hmk.set.list[i], "/", "HMK.kegg.full.csv", sep="")
  bp.file <- paste(hmk.set.list[i], "/", "HMK.bp.full.csv", sep="")
  cc.file <- paste(hmk.set.list[i], "/", "HMK.cc.full.csv", sep="")
  mf.file <- paste(hmk.set.list[i], "/", "HMK.mf.full.csv", sep="")
  ...
  hmk.summary <- rbind(hmk.summary, c(hmk.set.list[i], gene.number, gene.number.set, 
                       ...
                       max.betweenness, mbg,
                       max.closeness, mcg,
                       cor.deg.bet, cor.deg.clo, cor.bet.clo, 
...
write.table(hmk.summary, file="Summary.Hallmark.Sets.eigen.csv", row.names = F, col.names = F, sep=",", quote = T)
```
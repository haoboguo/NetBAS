...
write.table(cancer.matrix, file="oncosig.uponly.int.csv", sep=",", col.names=F,
            row.names=F, quote=F)
...
png("onco-onco-up-int.heatmap.png", width=12, height=11, res=600, units="in")
heatmap.2(cancer.matrix, col=my_palette, trace='none', breaks=colors, 
          ...
          margins=c(10,10), sepwidth=c(0.01,0.01), 
          sepcolor="grey", colsep=1:cancer.dim, rowsep=1:cancer.dim)
dev.off()

hm <- heatmap.2(cancer.matrix, dendrogram = "both")
hc.row <- as.hclust(hm$rowDendrogram)

pdf("onco-onco-up-dendrogram.pdf", width=10, height=6,paper='special')

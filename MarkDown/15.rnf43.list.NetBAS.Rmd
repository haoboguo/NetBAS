...
write.table(vec, file="hs.rnf43.list.bp.txt", col.names=F, row.names=F, quote=F)
```
...
output <- paste("ms02.human", "/", "rnf43.list", "/", "ms02.", p, ".bp.matrix.csv", sep="")
...
hspin <- matrix(as.numeric(unlist(read.table("hs.rnf43.list.bp.txt", header=F, sep=","))), nrow=bp.dim, ncol=conn.dim)
obs <- c(hspin)

perm <- c()
for (i in 1:100) {
    name <- paste("ms02.human", "/", "rnf43.list", "/", "ms02.", i, ".bp.matrix.csv", sep="")
...
write.table(z, file="hs.rnf43.list.bp.z.csv", sep=",", row.names=F, col.names=F, quote=F)
```
...
write.table(enriched, file="hs.rnf43.list.bp.enriched.csv", row.names=F, col.names=F, quote=F, sep="\t")

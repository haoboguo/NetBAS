...
## KEGG pathways 
kegg.file <- read.csv("../Data/human.kegg.csv", sep="\t", header=T, stringsAsFactors=F)
kegg.gene <- kegg$gene  #it should be changed to System for yeast pin
kegg.path <- kegg$pathway
kegg.cat <- unique(kegg$pathway)
kegg.dim <- length(kegg.cat)
...
# the gene list
path.count <- c()
path.vec <- numeric(length=path.dim)

    for (i in 1:length(gl.orf)) {
      orf.ith <- gl.orf[i]
      orf.freq <- gl.freq[i]
      orf.path <- kegg.path[which(kegg.gene %in% orf.ith)]
      for (j in 1:length(orf.path)) {
        na <- which(kegg.cat %in% orf.path[j])
        path.vec[na] <- path.vec[na] + orf.freq
      }
  }
path.count <- c(path.count, path.vec)


# The background
path.vec <- numeric(length=bp.dim)
...
path.count <- c(path.count, bp.vec)

path.A <- matrix(path.count, nrow = path.dim, ncol = 2)

write.table(paht.A, file="21.PvNP.path.csv", sep=",", col.names=F, row.names=F, quote=F)
